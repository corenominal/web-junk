...
$(document).ready(function()
{
	...
	$( document ).on( 'click', '.junk-info ul li a', function( e )
	{
		...
		$.get( source, function( data )
		{
			...
			data = data.replace('<textarea id="code" class="code code-default" readonly></textarea>', '');
			data = data.replace('<div id="junk-info" class="junk-info junk-info-default"></div>', '');
			data = data.replace('<textarea id="code" class="code code-inverted" readonly></textarea>', '');
			data = data.replace('<div id="junk-info" class="junk-info junk-info-inverted"></div>', '');
			$( '#code' ).val( data );
		} , 'text' );
	...
	$( document ).on( 'keydown', 'body[class!="nohide"]', function(e)
	{
		if( e.keyCode == 72 )
		{
			$( '#junk-info' ).toggle();
